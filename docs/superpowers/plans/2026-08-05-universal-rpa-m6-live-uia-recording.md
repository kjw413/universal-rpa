...
## 참고: 함께 발견됐으나 이 계획의 범위가 아니었던 것 (모두 해결됨)

- **`windows.drag`/`windows.scroll`이 UIA 타깃에서 동작하지 않음** — 해결(`fdd07c9`).
- **더블클릭이 관측되지 않음** — 해결(`1d0787e`). **의심했던 원인이 정반대였다.** 하네스에 대고 측정한 결과: 단일 클릭은 4/4로 도달하고 겨눈 좌표도 버튼 안이므로 전달·좌표는 문제가 아니었다. 두 클릭이 느린 게 아니라 **너무 빨랐다** — 원시 주입이 간격 0ms에서 0/4, 30ms에서 4/4다. pywinauto는 실패하는 양극단만 만들 수 있다: `double_click_input`은 두 사이클을 같은 순간에 밀어넣고, `click_input` 두 번은 각각 `GetDoubleClickTime`이 지날 때까지 대기하여 창을 넘겨버린다.

  이 결함이 오래 미확증으로 남은 이유도 함께 확인됐다: 해당 단계의 유일한 검증이 `element_exists`라, **클릭이 닿지 않아도 실행은 성공으로 보고**된다.
